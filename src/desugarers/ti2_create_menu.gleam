...
fn homepage_link(at_root_data: ConstructedAtRootData) -> Option(VXML) {
  case at_root_data.homepage_url {
    ...
    url ->
      Some(
        V(desugarer_blame(271), "a", [attribute("href", url)], [
...
fn page_link(
  ...
  V(desugarer_blame(307), "a", [href_attr], [
...
fn attribute(key: String, value: String) -> Attr {
  Attr(desugarer_blame(314), key, value)
}
...
fn string_to_text_node(content: String) -> VXML {
  T(desugarer_blame(322), [Line(desugarer_blame(322), content)])
}
  ...
  let row_constructor = fn(row: #(Option(VXML), Option(VXML))) {
    ...
    case left, right {
      ...
      _, _ -> {
        ...
        Some(
          V(desugarer_blame(375), "MenuRow", [attribute("class", "menu-row")], [
            left,
            right,
          ]),
        )
...
fn index_link(toc_word: String) -> VXML {
  V(desugarer_blame(392), "a", [attribute("href", "./index.html")], [
    V(desugarer_blame(393), "span", [attribute("class", "inhalts_arrows")], [
      string_to_text_node("<< "),
    ]),
